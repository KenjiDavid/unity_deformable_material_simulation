...
public class MeshDeformation_1_1 : MonoBehaviour
{

    Mesh mesh;

    public Chart_Script_1 chartData;
    ...
    void Start()
    {
        ...
        chartData = FindObjectOfType<Chart_Script_1>();
    ...
    void CreateVertices()
    {
        ...
        for (int i = 0, z = initPosZ; z <= zSize + initPosZ; z++)  //ejes x y z horizontales, eje y vertical
        {
            for (int x = initPosX; x <= xSize + initPosX; x++)
            {
                ...
                // Guardar la posición en los vértices de la malla,  el *2 se utiliza para que cuando se desplace la malla, los nodos hijos se desplacen igual que el padre
    ...
    //CAMBIAR MÁS ADELANTE MÉTODO LOCALIZACIÓN DE MUELLES
    ...
    void AssignNewVertices()
    {
        ...
        for (int i = 0; i < (xSize + 1) * (zSize + 1); i++)
        {
            ...
            if (obj != null)
            {
                ...
                if (AllowMesh0 == true && Mesh0Vertices != null)
                {
                    Mesh0Vertices[i] = nodePosition;
                    Mesh0Vertices[i].y = y + colliderRadius; 
                }
    ...
    //GRAFICAS FUERZA Y DESPLAZAMIENTO
    /*
    //variables para la gráfica
    // Crear un objeto vacío para la gráfica
    GameObject chartObject = new GameObject("LineChart");

    // Agregar el componente LineChart
