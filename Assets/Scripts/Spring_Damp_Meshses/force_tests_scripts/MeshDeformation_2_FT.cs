...
public class MeshDeformation_2_FT : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        canvas_2.UpdateSliderRange(minColliderRadius, maxColliderRadius);
        canvas_2.UpdateCurrentValue(colliderRadius);

        //te dice que tamaño y numero de nodos hay
        numNodes = (xSize + 1) * (zSize + 1);
        canvas_2.GridInfo.text = $"(Size Mesh {xSize*MeshScale}X{zSize * MeshScale}, number of nodes {numNodes})";
    ...
    void CreateVertices()
    {
        ...
        for (int i = 0, z = initPosZ; z <= zSize + initPosZ; z++)  //ejes x y z horizontales, eje y vertical
        {
            for (int x = initPosX; x <= xSize + initPosX; x++)
            {
                ...
                rb.mass = 0.001f;  //LA MASA INFLUYE EN LA MALLA MUCHO, SI HAY POCA MASA EL OBJETO PASA FACILMENTE
    ...
    void CreateSpringJoint(GameObject firstObject, GameObject secondObject, float k_spring, float c_damp)
    {
        ...
        springJoint.tolerance = 0.00025f;
    ...
    public void UpdateColliderRadius(float newValue)  
    {
        if (newValue <= maxColliderRadius && newValue >= minColliderRadius)
        {
            ...
            canvas_2.CollRadSlider.value = newValue; // Actualiza el Slider visualmente
...
public void UpdateAllowMesh0(bool isOn)
    ...
    public void RestartMesh()
    {
        ...
        if(colliderRadius > maxColliderRadius || colliderRadius < minColliderRadius)
        {
            ...
            canvas_2.CollRadSlider.value = colliderRadius; // Actualiza el Slider visualmente
            canvas_2.ColliderRad.text = rad.ToString("F2");
        }
