...
public class MeshDeformation_5_Beehive_Demo : MonoBehaviour
{
    ...
    public Canvas_Beehive canvas_1;
    ...
    //VARIABLES MALLA BEEHIVE
    private float xSizeLong = 0f;
    private float zSizeLong = 0f;
    private bool[] validVertices;    //me sirve para comprobar si es un nodo que se va a utilizar o no.
    private bool groupA = false;
    private bool groupB = true;
    public bool LinSpring0_ConfigSpring1 = false;
    ...
    void CreateShape()
    {
        ...
        CreateVerticesBee();
        CreateSpringNodes();  //SE TIENE QUE ADAPTAR AL NUEVO MESH
        //AvoidCollisionFunction();    //SE PUEDO O NO UTILIZAR
        CreateTriangles();
        AssignColors();
    ...
    void Calculatelong()  //poner mas tarde dentro de una función
    {
        xSizeLong = MeshScale * (float)Math.Cos((1f / 6f) * Math.PI); Debug.Log(xSizeLong);
        zSizeLong = MeshScale * (float)Math.Sin((1f / 6f) * Math.PI); Debug.Log(zSizeLong);
    }

    //CREAR LOS VERTICES DE LA MALLA BEEHIVE
    void CreateVerticesBee()
    {
        maxTerrainHeight = initPosY;  //me soluciona el problema de colores con máx. y min. preestablecidos
        vertices = new Vector3[(xSize + 1) * (zSize + 1)];
        validVertices = new bool[(xSize + 1) * (zSize + 1)];
        
        Calculatelong(); //me calcula la distancia entre nodos en el eje x y z
        ...
        if (chargeTemplate == true)
        {
            ...
            if (File.Exists(filePath))
            {
                ...
                vertices = JsonUtility.FromJson<WrapperBeehive>(json).array;
            }
            else
            {
        ...
        for (int i = 0, z = 0; z <= zSize; z++) //eje z arriba
        {
            bool groupInit;
            if (z == 0 || z % 2 == 0) groupInit = groupA;
            else groupInit = groupB;

            for (int x = 0; x <= xSize; x++) //eje x derecha
            {
                y = initPosY;
                ...
                if (groupInit == true)
                {
                    ...
                    if (chargeTemplate == false)  //si no se ha cargado ningu archivo, pasa por aqui y guarda la posición
                    {
                        Vector3 nodePosition = new Vector3(x * xSizeLong + initPosX, initPosY + (y - initPosY) * MeshScale, z * zSizeLong + initPosZ);
                    ...
                    validVertices[i] = true;  //nodo válido/necesario
                ...
                {
                    vertices[i] = new Vector3(0, 0, 0);  //estos son nodos que no importan al programa, pero no se eliminan por conveniencia del programa
                    validVertices[i] = false;  //nodo NO válido/necesario
                }
    ...
    //te calcula la cantidad de muelles necesarios, solo verificación
    void CalculateNumSprings()
    {
        int x = xSize;
        int z = zSize;
        if (x % 2 != 0) x++;
        ...
        //caso muelle 2
        Debug.Log("Hay " + (x * ((z / 2) - 1)) + " muelles en el caso 2");
    ...
    void CreateSpringNodes()
    {
        ...
        {
            if (validVertices[i] == true)
            {
                GameObject obj = objectVector[i];
                if (obj != null)
                {
                    //muelle 1, diagonal hacia la izquierda, x-1 x+1
                    if (i % (xSize + 1) != 0 && i < numNodes - (xSize + 1))
                    {
                        // Obtiene los dos primeros objetos vacíos
                        GameObject firstObject = objectVector[i];
                        GameObject secondObject = objectVector[i + xSize];
                
                        if (LinSpring0_ConfigSpring1 == false)  //deja cambiar entre linear spring y configurable spring
                        {
                    ...
                    //muelle 2, vertical, z+2
                    if (i < numNodes - ((xSize + 1) * 2))
                    {
                        // Obtiene los dos primeros objetos vacíos
                        GameObject firstObject = objectVector[i];
                        GameObject secondObject = objectVector[i + 2 * xSize + 2];
                    ...
                    //muelle 3, diagonal derecha, x+1 z+1
                    if ((i + 1) % (xSize + 1) != 0 && i < numNodes - (xSize + 1))
                    {
                        // Obtiene los dos primeros objetos vacíos
                        GameObject firstObject = objectVector[i];
        ...
        //CalculateNumSprings(); NO BIEN CALCULADO POR EL MOMENTO
    ...
    //crea el muelle, confugura sus parametros
    void CreateSpringJoint(GameObject firstObject, GameObject secondObject, float k_spring, float c_damp)
    {
        ...
        springJoint.tolerance = springTolerance;
        ...
        if (SpringAutoUpdate == true)
        {
            //Asignacion función/código reasignación valor muelle en función de la defomación (K, C, tol.)
            // Crear y asignar el controlador
            RefreshingSpringControl springController = firstObject.AddComponent<RefreshingSpringControl>();

            // Seleccionar el SpringJoint en base al Rigidbody conectado
            SpringJoint correctJoint = GetSpringJointByConnectedBody(firstObject, secondObject.GetComponent<Rigidbody>());

            if (correctJoint != null)
            {
                // Asignar el SpringJoint correcto al controlador
                springController.springJoint = correctJoint;
                //Debug.Log($"Asignado SpringJoint conectado a {secondObject.name} al controlador en {firstObject.name}");
            }
            else
            {
                Debug.LogError($"No se encontró un SpringJoint conectado a {secondObject.name} en {firstObject.name}");
            }
        }
    } //end create spring joint
    


    
    //crea el muelle configurable, configura sus parametros
    void CreateConfigurableJoint(GameObject firstObject, GameObject secondObject, float k_spring, float c_damp)
    {
        ...
        linearLimitation.limit = 2 * MeshScale;
        ...
        //activa tolerancia en muelle lineal y angular
        configurableJoint.projectionMode = JointProjectionMode.None;
        configurableJoint.projectionDistance = 0f;  //temporal, cambiar más tarde
        configurableJoint.projectionAngle = 0f; //temporal, cambiar más tarde
    ...
    void CreateTriangles()
    {
        int vert = 0;
        int tris = 0;
        triangles = new int[(xSize * zSize * 3) - xSize * 3];
        //Debug.Log("cantidad triangulos " + triangles.Length);
        if (AllowMesh0 == true)  Mesh0Triangles = new int[(xSize * zSize * 3) - xSize * 3]; //permitir triangles de la capa fantasma
        ...
        for (int i = 0, z = 0; z < zSize - 1; z++)
        {
            for (int x = 0; x <= xSize; x++)
            {
                ...
                if (validVertices[i] == true)
                {
                    //hace el triengulo de la izquierda
                    if (i % (xSize + 1) != 0)
                    {
                        triangles[tris + 0] = i;
                        triangles[tris + 1] = i + xSize;
                        triangles[tris + 2] = i + 2 * xSize + 2;
                        tris += 3;
                    }


                    //hace el triangulo de la derecha
                    if ((i + 1) % (xSize + 1) != 0)
                    {
                        triangles[tris + 0] = i;
                        triangles[tris + 1] = i + 2 * xSize + 2;
                        triangles[tris + 2] = i + xSize + 2;
                        tris += 3;
                    }
    ...
    public void SaveVector3ArrayToFile(Vector3[] vertices)
    {
        ...
        string json = JsonUtility.ToJson(new WrapperBeehive { array = vertices }, true);
