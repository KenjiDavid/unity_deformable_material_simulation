...
public class MeshDeformation_4_MultiCapa : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        canvas_4.UpdateSliderRange(minColliderRadius, maxColliderRadius);
        canvas_4.UpdateCurrentValue(colliderRadius);

        //te dice que tamaño y numero de nodos hay
        int numNodes = (xSize + 1) * (zSize + 1) * num_layers;
        canvas_4.GridInfo.text = $"(Size Mesh {xSize * MeshScale}X{zSize * MeshScale}, number of nodes {numNodes})";
                ...
                for (int i = 0; i < num_nodes_per_layer; i++)
                {
                    ...
                    if (obj != null)
                    {
...

                        {
                            ...
                            {
                                CreateConfigurableJoint(firstObject, secondObject, springConstantHorizontal * (float)Math.Sqrt(2), dampingConstantHorizontal * (float)Math.Sqrt(2));
                            }
                        ...
                        if (i < numNodesLayer - (xSize + 1))
                        {
                            ...
                            {
                                CreateConfigurableJoint(firstObject, secondObject, springConstantHorizontal, dampingConstantHorizontal);
                            }
                        }
    ...
    //me crea los muelles en las posiciones verticales
        ...
        for (int l = 2; l <= num_layers; l++)
        {
            ...
            if (meshDataDict.TryGetValue(meshName1, out var meshData1) && meshDataDict.TryGetValue(meshName2, out var meshData2))
            {
                ...
                for(int i = 0; i < num_nodes_per_layer; i ++)
                {
                    ...
                    //crea los nodos verticales directos (misma posicion horizontal)
                    // Obtiene los dos primeros objetos vacíos
                    GameObject firstObject = LayerObjectVector1[i];
                    GameObject secondObject = LayerObjectVector2[i]; //sera el que cambie, capa inferior
                    if (LinSpring0_ConfigSpring1 == false)//deja cambiar entre linear spring y configurable spring
                    {
                        ConfigureSpringNodes(firstObject, secondObject, (springConstantVertical + (l - 2) * springIncrementation),
                                                (dampingConstantVertical + (l - 2) * dampingIncrementation));
                    }
                    else
                    {
                        CreateConfigurableJoint(firstObject, secondObject, (springConstantVertical + (l - 2) * springIncrementation),
                                                (dampingConstantVertical + (l - 2) * dampingIncrementation));
                    }



                    //condiciones spring joint arriba (X=, Z+1) , posicion vector i + xSize + 1 (arriba)
                    if (i < numNodesLayer - (xSize + 1))
                    {
                        //Debug.Log("OK arriba, muelle entre Mesh Layer " + (l  - 1) + " y Mesh Layer " + l + " ,hay " + (numNodesLayer - xSize - 1));

                        // Obtiene los el nodo inferior primeros objetos vacíos
                        secondObject = LayerObjectVector2[i + xSize + 1];

                        float h = Vector3.Distance(firstObject.transform.position, secondObject.transform.position);
                        h = h / DistanceLayer;
                        ...
                        if (LinSpring0_ConfigSpring1 == false)//deja cambiar entre linear spring y configurable spring
                        {
                            ConfigureSpringNodes(firstObject, secondObject, (springConstantVertical + (l - 2) * springIncrementation) * h,
                                                    (dampingConstantVertical + (l - 2) * dampingIncrementation) * h);  //tiene en cuanta que es muelle diagonal
                        }
                        else
                        {
                            CreateConfigurableJoint(firstObject, secondObject, (springConstantVertical + (l - 2) * springIncrementation) * h,
                                                    (dampingConstantVertical + (l - 2) * dampingIncrementation) * h);  //tiene en cuanta que es muelle diagonal
                        }
                    }


                    //condiciones spring joint derecha (X+1, Z=) , posicion vector i + 1  (derecha)
                    if ((i + 1) % (xSize + 1) != 0)
                    {
                        //Debug.Log("OK derecha, muelle entre Mesh Layer " + (l - 1) + " y Mesh Layer " + l + "  ,hay " + (numNodesLayer - zSize - 1));

                        // Obtiene los dos primeros objetos vacíos
                        secondObject = LayerObjectVector2[i + 1];
                    ...
                    //condiciones spring joint abajo (X=, Z-1) , posicion vector i - (xSize + 1)  (derecha)
                    if (i > xSize)
                    {
                        //Debug.Log("OK abajo, muelle entre Mesh Layer " + (l - 1) + " y Mesh Layer " + l + "  ,hay " + (numNodesLayer - zSize - 1));
                    ...
                    //condiciones spring joint izquierda (X-1, Z=) , posicion vector i + 1  (derecha)
                    if (i % (xSize + 1) != 0)
                    {
                        //Debug.Log("OK izquierda, muelle entre Mesh Layer " + (l - 1) + " y Mesh Layer " + l + "  ,hay " + (numNodesLayer - zSize - 1));
    ...
    public void UpdateColliderRadius(float newValue)
    {
        if (newValue <= maxColliderRadius && newValue >= minColliderRadius)
        {
            ...
            canvas_4.CollRadSlider.value = newValue; // Actualiza el Slider visualmente
    ...
    public void AssignTempVariables()
    {
        ...
        if (colliderRadius > maxColliderRadius || colliderRadius < minColliderRadius)
        {
            ...
            canvas_4.UpdateCurrentValue(colliderRadius);
            colliderRadius = rad * 2;
            canvas_4.CollRadSlider.value = colliderRadius; // Actualiza el Slider visualmente
            canvas_4.ColliderRad.text = rad.ToString("F2");
        }
